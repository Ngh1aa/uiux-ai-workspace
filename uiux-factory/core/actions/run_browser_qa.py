...
class QuietHandler(SimpleHTTPRequestHandler):
    def end_headers(self):
        if (Path(self.directory) / ".uiux-ai.json").exists():
...
class RunBrowserQA(Action):
    ...
    async def inspect_page(
        ...
        context = await browser.new_context(
            ...
            # CSP sandbox already prevents workers for AI drafts. Playwright's
            # blocker injects a navigator.serviceWorker getter that throws there.
            service_workers="allow" if (project_dir / ".uiux-ai.json").exists() else "block",
        )
